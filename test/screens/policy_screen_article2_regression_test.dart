...
//   AC3. 「オプトアウト」の明示があること
//   AC4. オプトアウト導線として「マイページの『利用統計の提供』」が明示されていること
...
void main() {
  ...
  group('policy_screen — 第2条 デフォルトON/オプトアウト表記（回帰防止）', () {
    ...
    test('第2条にオプトアウト導線として「マイページの「利用統計の提供」」が明示されているとき'
        'ユーザーが迷わず停止できる', () {
      final article2 = _extractArticle2Body(_readPolicySource());
      // ポリシー文中の鍵括弧は「」で統一されている
      expect(
        article2.contains('マイページの「利用統計の提供」'),
        isTrue,
        reason: '第2条にオプトアウトの UI 導線（マイページの「利用統計の提供」）が明示されていません。\n'
            'settings_screen.dart の AnalyticsOptInTile ラベルと一致する文言を保ってください。\n'
            '文言が乖離するとユーザーがオプトアウト UI にたどり着けません。',
      );
    });
  });
