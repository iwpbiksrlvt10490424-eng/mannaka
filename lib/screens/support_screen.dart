...
class SupportScreen extends StatelessWidget {
  ...
  static const _howToSteps = [
    ...
    (
      ...
      '行ったお店に記録する',
      '「予定」画面の「決定！行った」ボタンを押すと行ったお店に記録されます。誰と行ったかグループ名も残ります。',
    ),
  ];
  ...
  static const _faqs = [
    ...
    (
      '探す画面の項目が多すぎる',
      '日程・シーン・個室などの細かい指定は「詳細条件」を開いてから入力できます。最初は駅・ジャンル・予算だけで検索できます。',
    ),
    (
      '気になるお店を複数人に相談したい',
    ...
    (
      '集合駅をLINEで共有したい',
      '結果画面の上にある集合場所カードの「この場所をLINEで共有」ボタンから、集合駅と参加者の移動時間をまとめてLINEで送れます。',
    ),
    (
      'LINEで送れるお店は何件？',
      '1回で送れるのは5件までです。お店カード右上の「+」で候補に追加すると、下部のバーから選択した順にまとめて LINE に送れます。',
    ),
    (
      'お店のリンクをタップすると何が見られる？',
    ...
    (
      '友達がアプリを持っていなくても使える？',
      'LINE 本文に駅ごとの候補と Google の店舗ページ URL が入るので、相手はアプリなしでも内容を読めます。相手にも Aimachi を使ってもらえば、同じ条件で自分で探せます。',
    ),
    (
      '過去に見たお店をもう一度見たい',
      '「履歴」タブには、過去の検索で詳細を開いた（タップした）お店だけが集合駅ごとに残ります。カードをタップすると、そのとき閲覧したお店の一覧を見返せます。表示しただけでタップしなかったお店は履歴に残りません。',
    ),
    (
      '行ったお店の記録を消したい',
    ...
    (
      '予約したお店はどこで確認できる？',
      '「予定」タブに保存されます。お店に行ったら「決定！行った」を押すと「行ったお店」に記録が移ります。',
    ),
    (
      '行ったお店を手動で追加したい',
      '履歴画面の「行ったお店」タブ右下の「+」から手動追加できます。予定画面も同様です。',
    ),
    (
      '「予約ページへ進む」を押すと何が起きる？',
      '外部の予約サイト（Hotpepper など）が開きます。Aimachi 内では予約は完結しません。予約後に「予定」タブに自動で記録されます。',
    ),
  ];
